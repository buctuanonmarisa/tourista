...
const travel_options_1 = require("../src/lib/travel-options");
const prisma = new client_1.PrismaClient();
const AVATAR_COLORS = ['ag', 'ac', 'ab', 'ap', 'ar', 'as', 'at', 'au', 'av', 'aw'];
const THUMBNAIL_COLORS = ['t1', 't2', 't3', 't4', 't5', 't6', 't7', 't8'];
const slugifyTag = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, '');
const stableImageLock = (value) => value.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
const travelQuery = (vlog) => `${vlog.location} ${vlog.country} ${vlog.vibe} travel vlog tour`;
const encodedTravelQuery = (vlog) => encodeURIComponent(travelQuery(vlog));
const coverPhotoUrl = (vlog) => `https://loremflickr.com/1200/800/${encodeURIComponent(vlog.location)},${encodeURIComponent(vlog.country)},travel/all?lock=${stableImageLock(vlog.title)}`;
...
const youtubeTravelUrl = (vlog) => `https://www.youtube.com/watch?v=${YOUTUBE_VIDEO_IDS[vlog.title] || 'sWEBl9A4lNY'}`;
const instagramTravelUrl = (vlog) => `https://www.instagram.com/explore/tags/${slugifyTag(`${vlog.location} ${vlog.country} travel`)}/`;
const tiktokTravelUrl = (vlog) => `https://www.tiktok.com/search?q=${encodedTravelQuery(vlog)}`;
const facebookTravelUrl = (vlog) => `https://www.facebook.com/search/videos/?q=${encodedTravelQuery(vlog)}`;
...
async function main() {
    ...
    await prisma.travelOption.createMany({
        data: Object.entries(travel_options_1.TRAVEL_OPTION_SEEDS).flatMap(([category, labels]) => labels.map((label, index) => ({
            category,
            label,
            value: label,
            sortOrder: index,
        ...
        const vlog = await prisma.vlog.create({
            data: {
                ...
                itinerary: template.itinerary ? {
                    create: template.itinerary.map((day) => ({
                        day: day.day,
    ...
    const buildItinerary = (data) => {
        const addGuide = (location, description, days, food = 'Use local markets, family-run restaurants, and neighborhood stalls near each stop.', transit = 'Use the standard local route for the day and budget extra time during peak hours.', tip = 'Check opening hours and weather the night before so the route stays realistic.') => {
            if (locationGuides[location])
                return;
    ...
    const seededBudgets = new Set([
        ...seededVlogInputs.map(v => {
            if (v.cost < 10000)
                return 'Under PHP 10k';
            if (v.cost <= 30000)
                return 'PHP 10k - PHP 30k';
    ...
    console.log(`   • Coverage across ${seededVibes.size}/${travel_options_1.FALLBACK_VIBES.length} frontend vibes`);
    console.log(`   • Coverage across ${seededCountries.size}/${travel_options_1.FALLBACK_COUNTRIES.length} frontend countries`);
    console.log(`   • Coverage across ${seededBudgets.size}/4 frontend budget filters`);
    console.log(`   • ${travel_options_1.FALLBACK_COUNTRIES.length + travel_options_1.FALLBACK_VIBES.length + travel_options_1.FALLBACK_BUDGETS.length} travel filter options saved to the database`);
